...
> ⚠️ 用 fork **`alexhegit/GraspGenX` 的 `rocm` 分支**（引用基准 = main+openarm+visual-mesh-demo 并集），不是上游。
> OpenArm 夹爪与 ROCm 支持上游均未合入，由该 fork 实现并提 PR：
...
> AMD 安装：`git clone -b rocm ... && uv sync --extra rocm`（`rocm` 与 `end2end` 互斥，见 ADR 0002）。
