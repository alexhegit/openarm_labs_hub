...
| `openarm_mp_labs` | **fork: alexhegit** | main（venv 已同步一致） | 运动规划 labs（pick-and-place 轨迹+MuJoCo 回放+双机位录制） | 仿真 / 运动规划 |
| `GraspGenX` | **fork: alexhegit** | rocm（引用基准：main+openarm+visual-mesh-demo 并集） | 跨形态 6-DOF 抓取生成（已 onboard openarm 夹爪 + ROCm + 视觉网格 demo） | 抓取生成 |
...
- ✅ 真实 3D 扫描物体 ginger（Scan2Sim）pick-and-place 跑通并 vendored 进 openarm_mp_labs（`--object ginger` 自包含运行）
- ✅ 抓取生成与仿真用**文件契约解耦**（ADR 0005）；新物体流程文档化 → `openarm_mp_labs/docs/new_object_pick_place.md`
- ⏳ 阶段 7（可选）：PyRoki IK 进 venv-planner
- ⏳ 后续：side-grasp 接触力附着、夹爪缓存提分、（可选）GraspGenX ZMQ 服务化做"一键新物体抓取"
